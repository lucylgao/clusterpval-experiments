...
make_two_clusters_mod <- function(n, q, len, sig) { 
  new_model(name="two-clusters-mod", 
            label=sprintf("Two clusters data (n=%s, q=%s, len=%s, sig=%s)", n, q, len, sig), 
            params=list(n=n, sig=sig), 
            simulate=function(nsim) { 
              lapply(1:nsim, function(x) generate_two_clusters_data(n, q, len, sig))
            })  
}
